...
`kde-live` needs the `kwin` package (`kwin_wayland`, `kpackagetool6`, `kwriteconfig6`) and nothing from the user's session: it builds its own private bus, XDG tree, and nested compositor. It is Synthetic session evidence whatever it reports. A nested `kwin_wayland` runs `Session::Type::Noop` and the outer compositor owns the login seat, so it cannot close the KDE Release gate; only a Smoke run on a seated KWin session does that.
...
| KDE Plasma Wayland | deferred - unverified | KDE Plasma Wayland with KWin 6.x | unverified. A nested KWin gate exists (`./scripts/smoke.sh kde-live`) but has not been run: `kwin_wayland` is not installed on the verification host. A nested run would be Synthetic session evidence and would leave this status unchanged. | — | Run a Smoke run on a seated KWin 6.x session; this closes the Release gate. |
