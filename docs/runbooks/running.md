...
On Wayland, native hotkeys are available when a supported compositor integration exists (Sway via `swaymsg`, Hyprland via `hyprctl`). When supported, global binds are registered through the Wayland backend and dispatched asynchronously while running; otherwise the process falls back to CLI hotkey mode (manual `dispatch`). If the native listener fails after startup (for example, after a compositor restart), runtime falls back to CLI mode for the current session.
