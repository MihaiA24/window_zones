...
- GNOME is split. A Synthetic session (`gnome-shell --headless` with two virtual monitors, isolated dconf and data directories, on a private session bus started under a PTY) carries Companion load, capability negotiation, display enumeration, and Usable area math. The seated live GNOME session carries focused-window discovery, placement, real accelerator capture, Companion disconnect and recovery, and the TUI lifecycle.
